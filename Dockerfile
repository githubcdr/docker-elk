...
RUN	   adduser -D -h /opt/elasticsearch elasticsearch \
	...
	&& tar -zxf logstash.tar.gz --owner=kibana --group=kibana --strip-components=1 -C /opt/logstash \
	&& tar -zxf kibana.tar.gz --owner=logstash --group=logstash --strip-components=1 -C /opt/kibana \
